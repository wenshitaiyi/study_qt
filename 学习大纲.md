...
#### 1.1.2 demo_widget
**描述**：基于 `QWidget` 的基础窗口程序，展示窗口坐标系、窗口标志位（WindowFlags）、窗口模态性与最简事件响应。  
**应用场景**：轻量级悬浮窗、桌面小挂件、内嵌到其他系统中的子窗口部件。

#### 1.1.3 demo_mainwindow
**描述**：基于 `QMainWindow` 的经典桌面应用框架，展示菜单栏（QMenuBar）、工具栏（QToolBar）、状态栏（QStatusBar）、中心部件（CentralWidget）以及停靠面板（QDockWidget）的协同工作机制。  
**应用场景**：IDE 编辑器、数据分析客户端、工业监控控制台等大型桌面主程序。

#### 1.1.4 demo_dialog
**描述**：展示模态对话框（`exec()` 阻塞）与非模态对话框（`show()` 异步）的区别，以及常用内置标准对话框（QFileDialog, QMessageBox, QColorDialog, QFontDialog, QInputDialog）的封装调用。  
**应用场景**：软件设置弹窗、文件保存与导出提示、用户登录鉴权确认框。

#### 1.1.5 demo_ui_usage
**描述**：系统演示 Qt 界面设计的 3 种主流加载方式：单继承（指针组合式）、多继承（直接继承 Ui 类）、动态加载（`QUiLoader` 无需编译直接解析 `.ui` 文件）。  
**应用场景**：跨团队协作时 UI 设计稿直接动态加载、Visual Studio / Qt Creator 生成代码风格的兼容适配。
...
#### 1.2.1 demo_debug
**描述**：使用 `qDebug`, `qInfo`, `qWarning`, `qCritical` 输出日志，并通过 `qInstallMessageHandler` 实现日志重定向写入本地文件，支持按日期滚存与格式化（包含时间戳、文件行号、线程ID）。  
**应用场景**：生产环境客户端的运行日志落盘、现场异常崩溃定位排查。

#### 1.2.2 demo_string
**描述**：演示 `QString`, `QByteArray`, `QStringView`, `QStringList` 的高效拼接、正则匹配、拆分替换，以及 UTF-8 / GBK / Local8Bit 编码转换与乱码规避。  
**应用场景**：解析文本报文、跨平台中文路径读取、串口/网络字节流与字符串互相转换。

#### 1.2.3 demo_containers
**描述**：对比 `QVector`/`QList`, `QMap`, `QHash`, `QSet` 的性能差异与隐式共享机制，展示 Java 风格与 STL 风格迭代器的使用与注意事项。  
...
#### 1.3.1 demo_file
**描述**：利用 `QFile`, `QFileInfo`, `QDir`, `QFileSystemWatcher` 进行文件读写、权限检查、目录递归遍历以及实时监控文件/目录变更。  
**应用场景**：日志文件读取、项目工程文件目录扫描、热重载配置文件监控器。

#### 1.3.2 demo_text_stream
**描述**：使用 `QTextStream` 按行、按格式读写文本文件，自动处理换行符（CRLF/LF）与多编码字符集。  
**应用场景**：读取 CSV 数据报表、导出系统巡检日志文本、导入脚本文件。

#### 1.3.3 demo_datastream
...
#### 2.4.1 demo_standard_item_model
